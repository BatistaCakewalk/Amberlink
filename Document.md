...
• New-line Based: No semicolons required. The parser uses significant newlines to end statements.
• Null-Safety: All types are non-nullable by default. Optional types must be explicitly declared (e.g., Int?).
• Multi-Paradigm: Supports both Object-Oriented Programming (OOP) via Classes and Functional Programming via standalone functions.
...
• Object Header: Only 8 bytes (Type Tag + Mark Bit).
• GC Strategy: A generational Mark-and-Sweep collector that can be manually "hinted" by the developer to pause during high-performance tasks.
